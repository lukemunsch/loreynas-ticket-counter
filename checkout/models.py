...
class Order(models.Model):
    ...
    original_wallet = models.TextField(
        null=False,
        blank=False,
        default=''
    )
    ...
    def update_total(self):
        ...
        self.total_count = self.lineitems.aggregate(
            Sum('quantity'))['quantity__sum'] or 0
        if self.total_count >= settings.DISC_THRESHOLD:
            self.discount = self.order_total * settings.DISC_PERC / 100
        else:
            self.discount = 0
...
class OrderLineItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        """override save method to set line
        item total price if not already set"""
